...
No environment file is required for local development. The API uses local PostgreSQL, Redis, port `8080`, and a non-secret development encryption key by default.

If your local services use different addresses, export only the values you need before running the API:

```bash
export APP_ENV=development
export HTTP_ADDRESS=:8080
export DATABASE_URL='postgres://business_drift:business_drift@localhost:5432/business_drift?sslmode=disable'
export REDIS_URL='redis://localhost:6379/0'
export ENCRYPTION_KEY='<base64-encoded-32-byte-key>'
make run
```

`ENCRYPTION_KEY` must decode to exactly 32 bytes. Generate and store a stable key for any environment where encrypted Stripe credentials must survive restarts. Production also requires explicit `DATABASE_URL`, `REDIS_URL`, and `ENCRYPTION_KEY` values.
